...
The connected legacy repository was inspected at:
...
| Corpus policy | Human-logic and pragmatic datasets are manifest-only/blocked until bytes, checksum, exact license text, and attribution are captured. Small local fixtures test importer behavior and count as fixture-only. |
